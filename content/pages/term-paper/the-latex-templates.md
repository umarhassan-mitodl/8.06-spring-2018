...
For Mac OS X users, the program TeXShop (available as part of the {{% resource_link "90d2438c-1149-49a3-8b7a-f912bdde9611" "MacTex package" %}}) can be a good alternative to the command line. TeXShop combines a text editor with a TeX compiler, and has the feature that Cmd-clicking on one window will jump to the corresponding point in the other window. For Windows users, LaTeX can be downloaded from the {{% resource_link "58eb206a-884c-4d45-8b02-7e7e8a948199" "MiKTeX project page" %}} and this includes a program TeXworks (also available on other platforms) with similar features to TexShop. On any platform, emacs has extensive LaTeX support, including a semi-WYSIWYG ability to preview equations within the editor.